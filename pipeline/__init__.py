...
def _get_pipeline_doc_store(pipeline):
    ...
    if isinstance(document_store, SINGLE_PROCESS_DOC_STORES):
        logger.warning(
            "FAISSDocumentStore or InMemoryDocumentStore should only be used with 1 worker."
        )
...
def setup_pipelines() -> Dict[str, Any]:
    ...
    query_pipeline, document_store = _load_pipeline(
        config.PIPELINE_YAML_PATH, config.QUERY_PIPELINE_NAME
    )
    ...
    index_pipeline, _ = _load_pipeline(
        config.PIPELINE_YAML_PATH, config.INDEXING_PIPELINE_NAME
    )
